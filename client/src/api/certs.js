...
export async function createCerts(file) {
  console.log("whatisthefile", file);
  const formData = new FormData();
  formData.append("file", "OI"); // 'file' is the File object obtained from the file input field
  console.log("idkkkkkk", formData);
  const fileData = { file: "hey" };
  return requestPost(CREATE_CERTS_API, { fileData });
}
