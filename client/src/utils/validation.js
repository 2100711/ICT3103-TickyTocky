...
export const validateCert = async (
  case_serial,
  movement_serial,
  dial,
  bracelet_strap,
  ...
  const isDupSerial = await checkDupSerial(serial); //TODO: USE THIS VARIABLE: ZAF
  ...
  console.log("ISDUPEMAIL", isDupEmail);

  if (!validateEmail(user_email)) {
    errors.push("Email provided must be gmail.");
  }

  if (!isDupEmail) {
    errors.push("Email provided does not exist.");
  }
  ...
  if (isDupSerial) {
    errors.push("Serial already exist.");
  }
  ...
  if (!isFutureDate) {
    errors.push("Expiry date cannot be current or past date.");
  }

  // Date validations
  if (date_of_validation >= issue_date) {
    errors.push("The Date of Validation must occur before the Issue Date.");
  }

  if (date_of_validation >= expiry_date) {
    errors.push("The Date of Validation must occur before the Expiry Date.");
  }

  if (issue_date >= expiry_date) {
    errors.push("The Issue Date must occur before the Expiry Date.");
  }

  if (!validateRemarks(remarks)) {
    errors.push("Remarks must not exceed 255 characters in length.");
  }
