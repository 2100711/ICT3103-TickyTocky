...
export function validateExcelData(data) {
    ...
    for (const row of data) {
        ...
        if (row.date_of_validation >= row.issue_date) {
            validationErrors.push("The Date of Validation must occur before the Issue Date.");
        }

        if (row.date_of_validation >= row.expiry_date) {
            validationErrors.push("The Date of Validation must occur before the Expiry Date.");
        }

        if (row.issue_date >= row.expiry_date) {
            validationErrors.push("The Issue Date must occur before the Expiry Date.");
        }

        if (!validateRemarks(row.remarks)) {
            validationErrors.push("Remarks must not exceed 255 characters in length.");
        }
    }
