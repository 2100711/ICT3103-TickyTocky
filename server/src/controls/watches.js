...
const createWatch = async (watch, session) => {
    try {
        const { case_serial, movement_serial, dial, bracelet_strap, crown_pusher, brand, model_no, model_name, movement, case_material, bracelet_strap_material, yop, gender } = watch;
        const sessionOptions = session ? { session } : {};
        const serialNumbers = { case_serial, movement_serial, dial, bracelet_strap, crown_pusher };
...
const updateWatch = async (watch, session) => {
    try {
        const { watch_id, case_serial, movement_serial, dial, bracelet_strap, crown_pusher, brand, model_no, model_name, movement, case_material, bracelet_strap_material, yop, gender } = watch;
        const sessionOptions = session ? { session } : {};
        ...
        const updatedWatch = await WatchModel.findByIdAndUpdate(watch_id, {
            brand,
            model_no,
            model_name,
            movement,
