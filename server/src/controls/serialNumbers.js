...
const updateSerialNumbers = async (serialNumbers) => {
    try {
        const updatedSerialNumbers = await SerialNumberModel.findOneAndUpdate(serialNumbers._id, serialNumbers, { new: true });
...
const deleteSerialNumbers = async (serialNumbers) => {
    try {
        const deletedSerialNumbers = await SerialNumberModel.findByIdAndRemove(serialNumbers._id);
